...
Recently, as Remote Direct Memory Access (RDMA) becomes popular in datacenter 
...
uses the VM hypervisor to track memory pages modified during handling each 
request on a primary VM and transfers these dirty pages to a backup VM. Live 
migration is also invoked for improving computer load balance and consolidating 
many VMs on fewer computers to save datacenter energy. It uses a similar 
technique as primary-backup. Despite much effort, both these two approaches 
consume substantial application down time (\eg, 8 seconds in 
vMotion~\cite{vmotion:atc05}) and network bandwidth during transfering memory.
