...
Infiniband~\cite{infiniband}) becomes popular within a datacenter, 
RDMA becomes a promising solution to address the \paxos performance challenge. 
A fastest type of RDMA operations called ``one-sided RDMA writes" (for short, 
\emph{WRITE} in the rest of this proposal) round-trip takes only about 3 
\us~\cite{pilaf:usenix14}, because it allows a local computer to directly 
writes to a remote computer's memory without involving OS kernel or CPU on the 
remote computer. This ultra low latency not only comes from its kernel 
bypassing feature, but also its dedicated network stack implemented in 
