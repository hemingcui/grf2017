...
We have two new discoveries in this study. First, unlike state-of-the-art 
understanding that triggering a concurrency bug require lots of 
repeated executions, some concurrency bugs can be triggered and exploited with 
high probability. For instance, in a MySQL privilege escalation 
attack~\cite{mysql-bug-14747}, we chose nested \texttt{select} statements to 
trigger this bug and corrupted another MySQL user's privilege table by only 18 
runs in average. Second, even many of the concurrency bugs we studied were 
fixed by upgrading software,it doesn't help to recover from the attacks because 
hackers may have already broken in. Therefore, studying existing known
concurrency bugs is still crucial.

\vspace{-.15in}\subsubsection{Motivation of our \xxx proposal} 
\label{sec:out-study}\vspace{-.075in}

State-of-the-art has not presented any effective approache to 
tackle concurrency attacks, probably due to three major difficulties.  
First, state-of-the-art may still consider concurrency bugs and attacks are 
orthogonal behaviors because people lack a rigorous model to describe how 
bugs propagate to vulnerable instructions. Second, detecting a concurrency bug 
requires a systematic approach that can carefully choose both inputs and thread 
interleavings. Third, people lack a runtime defense infrastructure to 
extensively study the behaviors of concurrency attacks. 
Therefore, this proposal addresses each difficulty with an objective. 
...
proposed objectvies focus on the security consequence of concurrency 
