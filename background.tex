\vspace{-.15in}\section{Research Background2} 
...
services~\cite{ellis:thesis,manos:hotdep10,scatter:sosp11}. \paxos
replicates the same application on a group of computers (or \emph{replicas}) 
and always enforces same inputs for this application across replicas, as long 
as a quorum (typically, majority) of replicas still work normally. Therefore, 
\paxos tolerates various types of faults (\eg, minor replica failures and packet 
loss).
...
RDMA appears a promising solution to address the \paxos performance problem. 
