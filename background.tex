...
that hackers can leverage concurrency bugs to construct \emph{concurrency 
...
for single-threaded programs. For example, taint-tracking~\cite{taintdroid:osdi10, lift:micro06, myers:information, 
...
kernels, and one bug bypassed security checks in a security library \libsafe 
and led to stack overflows. \S\ref{sec:model} will give two concrete bug
examples.

We have two new discoveries in this study. First, unlike 
...
high probability. For instance, in a MySQL concurrency bug we studied, we 
chose nested \texttt{select} statements to trigger this bug and corrupted 
another MySQL user's privilege table by only 18 runs in average. Second, 
even many of the concurrency bugs we studied were fixed by upgrading software, 
it doesn't help to fix the concurrency attacks because hackers may have already 
broken in. Therefore, studying existing know concurrency bugs are still crucial.
...
errors~\cite{conseq:asplos11}. \textbf{Objective 2} will leverage symbolic 
