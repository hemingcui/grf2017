...
\func{map}/\func{reduce}) to let computational providers write their 
...
extremely hard to verified. \textbf{Objective 3} (\chref{sec:obj3}) takles
...
Data leakage (or breach) is a top threat in 
cloud computing~\cite{top-threats,kazim2015survey}. In a data provider's 
perspective, both computation providers (\eg, the 2017 
iCloud account leakage caused by third-parties~\cite{icloud-breach}) and 
cloud providers (the 2013 Yahoo Cloud compromise~\cite{yahoo-dropbox-breach} and 
the 2014 JP Morgan Chase account leakage~\cite{yahoo-dropbox-breach}) have 
caused severe data leakage and huge losts. This proposal aims to preserve the 
data provider's privacy by going two directions. First, we propose two novel 
complimentary techniques in \textbf{Objective 1} (\kakute) and \textbf{Objective 
2} (fine-grained differential privacy) to protect privacy against the 
computational providers in private clouds. Second, we propose \textbf{Objective 
3} (a new privacy-preserving compiler) to protect privacy against the (public) 
cloud providers. By integrating the outcomes from all three objectives, data 
privacy will be effectively preserved.
