...
migration are two similar techniques for improving VM fault-tolerance. For 
instance, after serving each request, the primary-backup technique transfers all 
memory pages modified by applications (\ie, dirty pages) from the primary to 
the backup to make them run in sync. Both primary-backup and migration 
techniques have the problems of significant application downtime (\eg, 8 
seconds in a live migration system vMotion~\cite{vmotion:atc05}) and network 
bandwidth.

A key reason that causes these problems is that these approaches have only one 
actual application execution on one VM. Therefore, to transfer dirty pages to a 
remote computer, substantial time and network bandwidth consumption are 
unavoidable.
...
Fortunately, \paxos-based replication can construct multiple actual executions 
...
consumes 2*\vv{R} resources compared to unreplicated executions as it 
maintains a primary execution and a backup execution. Although typical 
\paxos-based replications consume 3*\vv{R} resources, our VM design 
...
major backup then periodically does checkpoints on application states and 
propagate the checkpoints to the standby backup. A checkpoint does not 
perturb the leader because the leader and the standby backup still form a 
majority and agree on requests rapidly. If the leader fails, \falcon gives 
the major backup a high priority to become the new leader.
...
leader invokes \textbf{Algorithm 1} to do a \vv{log(N)} traversal on the 
