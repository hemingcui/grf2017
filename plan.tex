...
effectively identify which concurency bugs will lead to attacks and we can 
safely ignore those won't do.

% P3: pattern.
The two aforementioned examples appear to be diverse, but they have three 
common elements in order to construct concurrency attacks. First, corrupted 
global memory is necessary (\eg, \texttt{dying} in \libsafe, and the 
\texttt{buf\_len} in \apache). Second, an attack must associate with a 
vulnerable instruction (\eg, \texttt{strcpy()} or \texttt{setuid()}). Third, 
the corrupted values must cause abnormal behavior of the vulnerable 
instructions. For example, the abnormal behavior in \libsafe is we must 
leverage the corrupted memory to ``bypass the security check", an abnormal 
control-flow of the execution. For \apache, we must directly corrupt 
