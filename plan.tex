...
on the trace. If a concurrency bug is detected, we check whether the trace 
...
\para{Preliminary results.} We have developed two preliminary building blocks for 
this detection approach. First, we have presented the path slicer (Algorithm 1)
...
We have detected seven new serious bugs and reported to developers, and they 
have confirmed or fixed these bugs. Second, we have presented a data race detector in 
SOSP 2011~\cite{peregrine:sosp11}. This race detector has also detected several new data races in 
...
\para{Open research challenges.} Towards the complete development of our detection 
approach, we foresee three open research challenges as our future work. 
First, how to generalize our existing path slicing approach to support all the five types of 
vulnerable instructions we collected (\S\ref{sec:attack-phase})? Second, given the same 
type of a vulnerable instruction, how to enhance our path slicer to precisely identify the vulnerable 
...
bugs? These challenges may require innovative co-design of the three building 
blocks, and we are confident that addressing each of these challenges will spur 
...
We have developed preliminary systems to address the first two challenges. For 
the first challenge, we have presented a transparent \smr system 
