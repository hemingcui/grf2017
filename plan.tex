...
We plan to achieve the second goal with \emph{path slicing}. Given a 
...
running into the same buggy schedules (buggys schedules are extremely rare 
...
replicas, retrieves the latest checkpoint of the application program, and 
restores the program's state on replicas. If only minor replicas fail, 
\smr's fault-tolerance strength makes the other replicas run.

To address the fourth challenge, we plan to leverage recent work to 
