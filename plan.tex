...
fine-grained diff privacy}\label{sec:scheduler}\vspace{-.075in}
...
can be determine.
runs the same differentially private percentile estimation algorithm on
the inputs to privately compute the 25-th and the 75-th percentile
(a.k.a, lower and upper quartiles) of the inputs.
...
current methods~\cite{code-verification}.
...
than
the transition cost. Figure~\ref{on-time} shows the example. Initially, only 
function b and d
are running in enclaves, but the compiler finds out that cost of running c in an 
enclave
is less than the transition cost (assume it to be 2), then the whole function a
will be compile as an enclave. In another case where running function c
takes a high cost, combinations of enclaves will not happen.
...
Figure~\ref{asynccall}
shows the idea of this technique. This technique makes use of the multi-core
...
integrate \kakute with fine-grained privacy (part of \textbf{Objective~2}), and 
S2 
will form an eco-system (part of \textbf{Objective~3}). In 
