...
\para{Preliminary results.} We developed a \falcon initial prototype in two 
...
\para{Future work.} We will further improve \falcon's practicality in two 
...
application can assign other \vv{R} numbers to a task when it submits this task 
to \tripod.

It is often easy for an application to determine which computation tasks 
require high availability and thus a bigger \vv{R} value. For instance, 
...
tasks with \vv{R=3}. Suppose Hadoop submits two tasks to \tripod's leader 
...
tasks (\vv{R=3}) for fault-tolerance and runs all MPI tasks as is (\vv{R=1}).}
...
The goal is to reserve enough resources for \tripod to replicate a task with 
\vv{R} copies, although some tasks have only \vv{R=1}.

In the third step, an application scheduler submits tasks to the leader 
controller. If \vv{R} is 1, the leader schedules the task without consensus and 
revokes \vv{R-1} resources specified in this task from the application. By 
doing so, the conservatively reserved resources are given back to \tripod. If 
\vv{R} is larger than 1, The leader controller then invokes a consensus on this 
task across controllers.

Once a majority of controllers agrees on executing a task, each controller 
does the fourth step. It schedules this task on an available slave machine 
according to the resource offer. To prevent controllers putting the same task 
on the same slave machine, the leader controller first makes an assignment on 
which controller should run this task on which slave machine, it then carries 
this assignment in its consensus request. Once a consensus on this task is 
...
using \vv{R-1} more resources than that consumed by the application's essential 
...
developers to manually assign \vv{R} values to application components. We will 
...
Primary-backup and live migration are two similar 
techniques (\S\ref{sec:others-work}) for improving VM fault-tolerance. For 
instance, after serving every request, the primary-backup technique transfers 
all memory pages modified by applications (\ie, dirty pages in 4KB) from the 
primary to backup. This technique consumes 2*\vv{R} resources (\vv{R} denotes 
the amount of resources consumed by unreplicated execution). The transfered 
dirty bytes can be up to 1.7GB (\S\ref{sec:tracking}), causing a substantial  
delay and network bandwidth consumption for every request.

A key reason for this problem is that existing techniques have only one actual 
...
\vspace{-.15in}\subsubsection{Integrating \falcon with KVM} 
...
We proactively design our VM to achieve same 2*\vv{R} resource consumption as 
existing VM fault-tolerance techniques. Although \paxos 
normally consumes 3*\vv{R} resources, our VM design reduces this consumption 
...
the standby backup is elected a new leader, it extracts checkpoints and 
uses the agreed inputs to catch up with old leader quickly.
