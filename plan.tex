...
differential privacy (\ie, \textbf{Objective 2}) and
\func{k}-anonymization~\cite{kanonymity,icde06:ldiversity}, which will likely 
lead to the inventions of diverse privacy-preserving techniques and 
systems. 
...
privacy budgets and security levels (\vv{getTagLevel} in \textbf{Algorithm 1}). 
...
along three directions. First, we will further optimize the algorithm and 
reduce its error (inaccuracy). In \textbf{Algorithm 1}, the error of FDP's
results comes from two parts: the Laplace noise error and the 
partition error. It is crucial to reduce the final error incurred by this 
algorithm while keeping the differential security guarantee. The error can be 
reduced by by proposing other new algorithms (\eg, algorithms with 
hill-climbing or simulated annealing styles) to compute an optimal number of 
partitions. Second, we will do an extensive study on real-world big-data 
queries and quantify the accuracy improvements of FDP compared to existing 
differential privacy techniques (\eg, GUPT~\cite{gupt:sigmod12}). Third, 
...
particular data into those without (\eg, 
oblivious executions~\cite{bigmatrix:ccs17}). This will nurture new theory and 
algorithms on oblivious executions. Third, we will further enhance DAE to 
