...
optimal number of partitions for minimizing added noise. Second, we will 
study FDP's improvements compared to existing 
differential privacy techniques (\eg, GUPT~\cite{gupt:sigmod12}). Specifically, 
we will conduct an extensive study on diverse real-world big-data queries and 
quantify the accuracy improvements of FDP and its new algorithms on realistic 
datasets, including both sensitive and insensitive ones.
