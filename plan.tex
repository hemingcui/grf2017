...
technique to track the propagation (\S\ref{sec:model}) from a concurrency bug 
...
static analysis (\S\ref{sec:others-work}) on the non-executed branches to 
detect vulnerable instructions. The path slicer then asks symbolic execution 
to explore these branch instructions with high priority to increase the 
chance hitting vulnerable instructions in following explorations.
