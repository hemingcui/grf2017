...
instruction, or (2) this trace have any control statements that can lead to 
...
%     \centering
    \begin{minipage}{.5\textwidth}
        \centering
        \includegraphics[width=0.3\textheight]{figures/detection}
        \caption{Workflow}
        \label{fig:prob1_6_2}
    \end{minipage}%
...
enforces a different \dmt policy. If minor replicas fail, \smr's 
...
year, S1 will build the runtime defense infrastructure (\textbf{Objective 3}), and S2 
will apply our model, detection tool, and defense infrastructure to benefit 
related research areas (\eg, byzantine fault tolerance~\cite{pbft:osdi99}).
% Both students will 
