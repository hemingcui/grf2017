...
\para{Future work.} Above model is based on our preliminary study on \nattacks 
concurrency attacks. We will further study 10 times more concurrency attacks 
and refine our model with these research questions: (1) Are there other 
types of vulnerable instructions? (2) Is there malicious data propagation among 
vulnerable instructions? (3) How does a program's concurrency attack affect other programs? 

%so that we can collect more types of vulnerable instructions and make the 
%model's elements more comprehensive. During this further study, we will
...
anlysis tools. Symbolic execution marks a program input and bytes 
...
contains a vulnerable instructoin: if yes, we report a concurrency attack; if 
...
Algorithm 1 shows the algorithm of the path slicer, our core 
...
\para{Preliminary results.} We have presented a preliminary path slicer (Algorithm 1)
in ASPLOS 2013~\cite{woodpecker:asplos13} for detecting security bugs on one type of 
vulnenarable instructions: privilege operations (\eg, \texttt{setuid()}). 
This path slicer has detected seven new security bugs in widely used programs.

%Second, we have presented a precise data 
%race detector in SOSP 2011~\cite{peregrine:sosp11}. This data race 
%detector has detected several new data races in heavily-tested programs~\cite{wu:pldi12}.
...
\para{Future work.} We foresee three open research questions for our detection approach. First, how 
to generalize our existing path slicer to support all the five types of 
vulnerable instructions we collected (\S\ref{sec:attack-phase})? Second, given the same 
type of a vulnerable instruction, how to enhance our path slicer to identify the vulnerable 
ones over the safe ones (\eg, some \texttt{strcpy()} functions are just 
memory manipulation and do not involve inputs)? Third, how to guide symbolic execution 
to explore malicious inputs and thread interleavings that are more likely to trigger concurrency 
bugs? Addressing these challenges will probably lead to innovative techniques.
...
in it \emph{transparently} without modifications. The major benefit of 
transparency is that program developers can focus on implementing their 
...
\para{Preliminary results.} We have developed preliminary systems to address the 
first and the second challenges. For the first challenge, we have presented a transparent \smr system 
\crane~\cite{crane:sosp15} in SOSP 2015. \crane is able to transparently 
support widely used server programs with 34.2\% overhead in average 
(see Figure~\ref{fig:normalize-perf}). For the second challenge, we have 
presented an efficient \dmt runtime \parrot~\cite{parrot:sosp13} in SOSP 2013, 
which incurred merely 12.7\% overhead on a wide range of popular multithreaded 
programs.
