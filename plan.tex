...
providers do not need to be trusted. Figure \ref{fig:maat-arch} shows the 
architecture of \maat, in which both the translation of self-defined code and 
the execution of code are protected by enclaves. Even if a cloud's 
OS compromised, the OS can neither observe the data being queried nor inject 
malicious code into the queries during \maat's translation. \maat's code cache 
is for reusing translated code.
