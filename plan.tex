...
Despite recent systems (Opaque, VC3, SGX-BigMatrix) on building SGX-based 
...
The purpose of \maat is to keep confidentiality of data processed in enclaves, 
while other attacks such as changing of execution path and Denies of Services 
(DoS) are not our concerns, because we tend to defend against a public, 
malicous cloud proficer. There is an orthogonal approach that hardens the 
security of JVM itself~\cite{jitguard:ccs17}. \maat works as a SGX-aware java 
JIT compiler which automatically compiles self-defined big-data functions 
(\eg, \func{map}/\func{reduce}) into enclave-compatible assembly code. 
Therefore, the JVM itself does not need to be trusted, and people can run 
unmodified big-data Java algorithms in \maat with a strong privacy protection.


Figure~\ref{fig:maat-threat} shows the threat model of \maat. All grey 
boxes are not trusted and red lines represent potential leakages or attacks. 
Data is encrypted outside enclave and is in blue. Self-defined big-data query 
functions are treated as secured functions by \maat and they are compiled by a 
JIT translator in a enclave, which can ensures the integrity of the big-data 
query code. The compiler compiles the functions into enclave code, so that the 
program will call into a enclave when calling 
into this function. The actual executions of these self-defined functions

Figure \ref{fig:maat-arch} shows the architecture of \maat. Self-defined 
functions and data processed by these functions should be kept as secrets.
The secure functions are compiled and executed inside enclaves so that data and 
code will not be leaked. The Java byte-codes of these function are compiled to 
native enclave codes, and are executed inside enclaves using a standard enclave 
ECALL upon function calls of the running program.
...
assembly code. The management is for our own use of encryption/decription on 
data records and maintaining SGX memory for the queries at runtime. SGX is for 
protecting data and code running in enclaves, but code inside 
...
time-consuming with current verification methods (\chref{sec:others-work}).
...
One subtle issue for \maat is that it should have reasonable performance 
