...
from the primary to backup. This technique consumes 2*\vv{R} resources (\vv{R} 
...
We proactively design our VM replication architecture to maintain same 2*\vv{R} 
resource consumption as existing VM fault-tolerance techniques. Although \paxos 
consumes 3*\vv{R} resources, our VM design reduces this consumption by letting 
...
agrees on requests without executing them (\ie, without consuming \vv{R} 
resources). In total, 2*\vv{R} resources are consumed by the leader and major 
...
%     \end{minipage}
    \begin{minipage}{0.47\textwidth}
        \vspace{-.15in}
        \includegraphics[width=0.3365\textheight]{figures/tree.ps}
        \vspace{-.5in}
