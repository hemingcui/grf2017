...
\para{Research plan.} Since \falcon is the keystone of our objectives, this 
...
It is often easy for an application to identify which computation jobs require 
...
\para{Research plan.} Our \tripod development will go along two directions. 
...
augmented \falcon protocol as \textbf{FALCON()}, which returns divergent nodes 
...
%     \end{minipage}
    \begin{minipage}{0.47\textwidth}
        ...
        \vspace{-.4in}
        \caption{Data structures on tracking divergent pages between replicas. 
...
$root$.right()) \;
        \For {{\rm $i$ $\leftarrow$ $diverged$.push\_front()}} {
          ...
          \Else {
            $diff$ $\leftarrow$ \textbf{FALCON}($i$, $i$.left(), 
$i$.right()) \;
...
To verify whether divergent dirty pages are rare, we ran three real-world 
appliations, MySQL~\cite{mysql}, MongoDB~\cite{mongodb}, and 
Memcached~\cite{memcached}, on two KVMs and ran same real-world 
workloads~\cite{sysbench} on both VMs. We found that 6,010 to 14,295 
application memory pages were dirty. Existing primary-backup techniques need to 
transfer all these dirty memory from primary to backup. We also found that only 
471 to 983 of these dirty pages were divergent between two VMs. This preliminary 
study implies that our algorithm can reduce up to 96.7\% transfered pages 
compared to existing primary-backup techniques.
...
\para{Research plan}. We will fully implement this new fault-tolerant VM, 
